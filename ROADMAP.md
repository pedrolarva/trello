...
- User value proposition: Enables users to visually categorize cards, making it easier to scan boards and identify task types or priorities at a glance.
- Technical approach: Add a 'labels' array to the card object. Update the card rendering to display colored badges. Create a modal to select from predefined colors and add custom label names.
- Success criteria: Users can add, edit, and remove multiple colored labels on any card, and the labels are visible on the board view.
- Estimated effort: Medium

### 6. Search and filter cards by text or label ([#8](https://github.com/pedrolarva/trello/issues/8))
- User value proposition: Helps users quickly find specific tasks in large boards, reducing time spent scrolling and searching manually.
- Technical approach: Implement a search bar that filters the visible cards in the DOM based on text matching in the title/description or by selected labels. Non-matching cards will be hidden using CSS `display: none`.
- Success criteria: Searching instantly filters the board view to show only relevant cards without reloading the page.
- Estimated effort: Small

### 7. Theme customization (Dark Mode) ([#11](https://github.com/pedrolarva/trello/issues/11))
- User value proposition: Reduces eye strain in low-light environments and allows users to personalize the look and feel of their workspace.
- Technical approach: Use CSS variables for all color values. Implement a toggle that switches a data attribute on the `body` tag to redefine these CSS variables for dark mode. Save the preference in `localStorage`.
- Success criteria: Users can toggle dark mode, the entire UI updates smoothly, and the preference persists across sessions.
- Estimated effort: Small

### 8. Markdown Support in Card Descriptions ([#12](https://github.com/pedrolarva/trello/issues/12))
- User value proposition: Enables users to format text, create checklists, and add links within card descriptions for better task documentation.
- Technical approach: Integrate a lightweight markdown parser. Sanitize the HTML output to prevent XSS vulnerabilities before rendering it in the DOM. Update the edit modal to allow toggling between edit and preview modes.
- Success criteria: Users can write markdown in the description field and see correctly formatted HTML when viewing the card.
- Estimated effort: Medium
...
- Security with Markdown Parsing: When introducing markdown support, there is a risk of XSS attacks if user input is not properly sanitized. We must strictly use a robust sanitization library (like DOMPurify) when rendering HTML.
