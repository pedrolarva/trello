...
- High priority items (bugs, critical features)
  - [#1](https://github.com/pedrolarva/trello/issues/1) Bug fixes for Drag and Drop on mobile devices (touch events support).
...
- High priority items (bugs, critical features)
  - [#6](https://github.com/pedrolarva/trello/issues/6) Export/Import data (JSON) functionality for secure backups.
- Medium priority items (enhancements, improvements)
...
- High priority items (bugs, critical features)
  - [#10](https://github.com/pedrolarva/trello/issues/10) Full support for multiple boards.
- Medium priority items (enhancements, improvements)
...
- User value proposition: Allows users to easily back up their boards and transfer data between browsers or devices manually, ensuring they don't lose information if `localStorage` is cleared.
- Technical approach (high-level): Create functions to reliably serialize the current `localStorage` state into a `.json` file for download. Create a file input to read a `.json` file and update `localStorage`.
- Success criteria: The user can download the complete data as a JSON file and successfully restore it in a clean browser session.
...
## Dependencies & Risks
- LocalStorage Limitations: Browser local storage has a size limit (typically 5MB) and can be accidentally wiped by the user when clearing browser cache data. We may need to investigate IndexedDB for larger limits.
