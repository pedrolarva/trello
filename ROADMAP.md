...
## Vision & Goals
The primary vision of this project is to provide a fast, simple, and privacy-focused (local-first) web-based Kanban board built entirely using vanilla HTML, CSS, and JavaScript. Our goal is to deliver an intuitive and responsive user interface for personal or small-team task management, eliminating the need for complex server setups or backends. We focus heavily on practical, incremental value delivery to continuously improve the tool over time.

## Current Status
At present, the project has successfully implemented the following core functionalities:
- Create, edit, and delete lists (boards).
- Create, edit, and delete cards.
- Drag and Drop capabilities to smoothly move cards across lists.
- Reliable data persistence utilizing the browser's native `localStorage`.
- A fully responsive design featuring horizontal scrolling for wide boards.
...
- Medium priority items (enhancements, improvements)
  ...
  - [#17](https://github.com/pedrolarva/trello/issues/17) Checklists on cards to manage subtasks efficiently.
- Low priority items (technical debt, optimizations)
...
- Medium priority items (enhancements, improvements)
  ...
  - [#18](https://github.com/pedrolarva/trello/issues/18) Board Templates for quick project setups.
- Low priority items (technical debt, optimizations)
...
- User value proposition: Enables users to break down larger tasks into smaller, manageable subtasks directly within a card.
- Technical approach (high-level): Add a checklist array property to the card object. Create UI elements in the card modal to add, edit, and toggle completion of checklist items.
- Success criteria: Users can add multiple checklist items, toggle their status, and view a progress bar or ratio on the card preview.
- Estimated effort: Medium

### 6. Board Templates ([#18](https://github.com/pedrolarva/trello/issues/18))
- User value proposition: Provides users with pre-configured boards (e.g., Kanban, Scrum, Weekly Planner) to quickly start new projects without setting up lists manually.
- Technical approach (high-level): Store predefined board structures in a constant or separate JSON file. Create a new board modal where users can select a template, which then clones the structure into a new board instance in `localStorage`.
- Success criteria: Users can create a new board from a selection of templates and see the lists instantly populated.
- Estimated effort: Medium
