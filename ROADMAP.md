...
- **Dependencies & Risks:** Browser local storage limits; handling malformed JSON on import.
...
- **Dependencies & Risks:** Complex migration of existing single-board data structure; potential performance issues with large state objects.
...
- **Dependencies & Risks:** Date format differences across browsers; handling time zones correctly.

### Optional Cloud Sync [#14]
...
- **Estimated effort:** Large
- **Dependencies & Risks:** Requires an external service integration; risk of data conflicts if offline edits occur concurrently on different devices.
