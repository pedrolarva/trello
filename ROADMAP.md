...
- Medium priority items (enhancements, improvements)
  ...
  - Card Checklists to track sub-tasks.
- Low priority items (technical debt, optimizations)
...
### 5. Card Checklists
- User value proposition: Allows users to break down complex tasks into smaller, manageable sub-tasks within a single card.
- Technical approach (high-level): Add a new property 'checklists' to the card object. Build a UI in the card detail modal to add, edit, delete, and toggle checklist items. Update the card summary to show the fraction of completed sub-tasks.
- Success criteria: The user can create checklist items on a card, mark them as complete, and view progress on the card face.
- Estimated effort: Medium
