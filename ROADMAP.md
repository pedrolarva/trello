...
### 5. Colored Labels/Tags ([#3](https://github.com/pedrolarva/trello/issues/3))
- User value proposition: Enables users to visually categorize and quickly identify cards using color-coded labels.
- Technical approach (high-level): Add an array of label colors to the card data structure. Create a UI element in the card modal to toggle labels, and display small colored indicators on the card face.
- Success criteria: The user can assign one or more colors to a card, and the colors are visible on the board and saved in `localStorage`.
- Estimated effort (Small/Medium/Large): Small
